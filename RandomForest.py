...
"""
During the whole project we will follow the PEP8 style, for more information use the next link:
                                        https://pep8.org/ 
"""
...
class RandomForest(ABC):
    ...
    _combine_predictions(predictions):
        abstract method, according to the type of random forest we are applying, it realizes a type of combination
    predict(X):
        will perform predictions on the testing instances
    _make_decision_trees(dataset) :
        creates the list of decision trees applying the criteria
    _make_node(dataset, depth) :
        creates the nodes of the trees, and classifies if are leaf or we still do not know if are parent or leaf
    _make_parent_or_leaf(dataset, depth) :
        calculates if the node entering is a parent or a leaf, and classifies that
    _make_leaf(dataset) :
         abstract method, that according to the type of random forest we are applying the leaf will create with
         appropriate method
    calculate_impurity(dataset) :
        calculus of the impurity selected
    _CART_cost(left_dataset, right_dataset) :
        minimizes the function cost
    feature_importance(num_features) :
        decides which feature to choose and the threshold to apply to it in order to follow the left or right
        branch. A simple way to derive importance is to take all the nodes from all the trees and count
        how many times each feature is used.

    print_trees:
        print, and show the trees created in the random forest

    """

    def __init__(self, max_depth: int, min_size_split: int, ratio_samples: float,
                 num_trees: int, num_random_features: int, criterion: Impurity, optimization: bool):
        ...
        criterion : Impurity
            the function to measure the quality of a split.
        optimization: bool
            it's a bool that lets you decide whether to use or not use optimization
    ...
    def _best_split(self, idx_features: np.ndarray[int], dataset: Dataset) -> Tuple[int, float, float,
                                                                                    List[Dataset]]:
            ...
            for idx in idx_features:
                ...
                if cost < minimum_cost:
                    best_feature_index, best_threshold, minimum_cost, \
                        best_split = idx, random, cost, [left_dataset, right_dataset]
        else:
            for idx in idx_features:
    ...
    def _CART_cost(self, left_dataset: Dataset, right_dataset: Dataset) -> float:
        ...
        """
        left_cost = left_dataset.num_samples / (left_dataset.num_samples + right_dataset.num_samples) * \
            self._impurity(left_dataset)
        right_cost = right_dataset.num_samples / (left_dataset.num_samples + right_dataset.num_samples) * \
            self._impurity(right_dataset)
    ...
    def feature_importance(self) -> Dict:
        """
        Mesures how important is each of the features counting how many times each feature is used
...
class RandomForestClassifier(RandomForest):
    ...
    def _combine_predictions(self, predictions: np.ndarray[float]) -> int:
        ...
        # type hint error because we cant specify that the array is one-dimensional
...
class RandomForestRegressor(RandomForest):
    ...
    def _combine_predictions(self, predictions: np.ndarray[float]) -> np.ndarray[float]:
        """
...
class IsolationForest(RandomForest):
    ...
    def _combine_predictions(self, predictions: np.ndarray[float]) -> int:
        ...
        ----------
        Score : float
            value that determines the probability of being an anomaly, the value is always between 0 and 1,
                if it's near 0, is probably a normal value, if it's near 1, it might be an anomaly
        """
        ehx = np.mean(predictions)
        cn = 2 * (np.log(self.train_size - 1) + 0.57721) - 2 * (self.train_size - 1) / float(self.test_size)
        return 2 ** (-ehx / cn)

    def _best_split(self, idx_features: np.ndarray[int], dataset: Dataset) -> Tuple[int, float, None, List[Dataset]]:
        """
        Find the best pair (feature, threshold) by exploring all possible pairs
        ...
        multiplier = np.random.random()  # This code line does a random value between 0 and 1
